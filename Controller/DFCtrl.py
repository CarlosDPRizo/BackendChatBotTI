...
async def processar_intencoes(req):
    if req.method == "POST" and req.is_json:
        ...
        if intencao == "InformarProtocoloNome":
            ...
            resposta = await devolver_escolhas(dados, origem)
        elif intencao == "AvaliarConversa":
            resposta = await registrar_log(dados, origem)
...
async def processar_escolha(dados, origem):
    ...
    for inf in infos_selecionadas:
        info = Info()
        resultado = await info.consultar(inf)
...
async def devolver_escolhas(dados, origem):
    ...
    for inf in info_selecionadas:
        busca = await info_model.consultar(inf)
        if busca:
